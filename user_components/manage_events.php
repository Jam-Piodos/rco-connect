    ...
    if ($end_time <= $start_time) {
        ...
        if ($conflicts) {
            ...
            if ($stmt->execute()) {
                ...
                header("Location: " . $_SERVER['PHP_SELF'] . "?action=added&t=" . time());
                exit();
            } else {
        ...
        if ($event && $event['created_by'] == $_SESSION['user_id']) {
            ...
            if ($conflicts) {
                ...
                if ($stmt->execute()) {
                    ...
                    // Redirect to refresh the page and show updated data
                    header("Location: " . $_SERVER['PHP_SELF'] . "?action=updated&t=" . time());
                    exit();
                } else {
...
if (isset($_POST['delete_event'])) {
    ...
    if ($event) {
        ...
        try {
            ...
            // Redirect to refresh the page and show updated data
            header("Location: " . $_SERVER['PHP_SELF'] . "?action=deleted&t=" . time());
            exit();
        } catch (Exception $e) {
...
if (errorAlert) {
    // Add a close button to error messages
    ...
    errorAlert.prepend(closeBtn);
    
    // Close button functionality
    closeBtn.addEventListener('click', function() {
        errorAlert.style.display = 'none';
        // Reset form on error close if it's related to adding an event
        if (errorAlert.textContent.includes('End time must be after start time') || 
            errorAlert.textContent.includes('Time conflict detected')) {
            document.getElementById('add-event-form').reset();
        }
    });
    
    // Auto-dismiss after longer period
    setTimeout(() => {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('add-event-form').addEventListener('submit', function(e) {
        ...
        if (endTime <= startTime) {
            ...
            // If form submission is successful, the page will redirect,
            // but we'll also reset the form in case the submission fails
            setTimeout(() => {
                this.reset();
            }, 500);
        }
    ...
    // Clear add event form if already submitted successfully (verified by success message)
    ...
    // Set focus to title field on page load if form is empty
    if (document.getElementById('title').value === '') {
        document.getElementById('title').focus();
    }
    
    // Highlight newly added/updated/deleted event based on URL parameter
    const urlParams = new URLSearchParams(window.location.search);
    const action = urlParams.get('action');
    
    if (action) {
        // Scroll to top for better visibility of success message
        window.scrollTo(0, 0);
        
        // For added/updated events, highlight the newest event
        if (action === 'added' || action === 'updated') {
            const eventCards = document.querySelectorAll('.event-card');
            if (eventCards.length > 0) {
                const firstEvent = eventCards[0];
                firstEvent.style.boxShadow = '0 0 10px #4CAF50';
                firstEvent.style.animation = 'highlight-pulse 2s';
                
                // Add the animation if it doesn't exist in the stylesheet
